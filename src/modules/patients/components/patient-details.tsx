      ...
      <div className="spd-pn">
        <SectionComponent title={text(`Basic Info`)}>
          <div className="name">
            <TextField
              ...
              onChange={(ev, name) => (this.props.patient.name = name!)}
            ...
            <Col sm={12}>
              <div className="birth">
                <TextField
                  ...
                  onChange={(ev, year) =>
                    (this.props.patient.birthYear = num(year!))
                  }
                  type="number"
                  disabled={!this.canEdit}
                />
        ...
        <SectionComponent title={text(`Contact Info`)}>
          <TextField
            ...
            onChange={(ev, phone) => (this.props.patient.phone = phone!)}
            type="number"
            disabled={!this.canEdit}
          />
          ...
          <TextField
            ...
            onChange={(ev, phone) => (this.props.patient.whatsapphone = phone!)}
            type="telephone"
            disabled={!this.canEdit}
          />
          ...
          <TextField
            ...
            onChange={(ev, email) => (this.props.patient.email = email!)}
            disabled={!this.canEdit}
          />
          ...
          <TextField
            ...
            onChange={(ev, address) => (this.props.patient.address = address!)}
            disabled={!this.canEdit}
          />
        </SectionComponent>
                ...
                options={[""]
                  .concat(
                    ...patients.list.map((patient) =>
                      patient.labels.map((label) => label.text)
                    )
                  )
                  .map((x) => ({
                    key: x,
                  ...
                  .reduce((arr: { key: string; text: string }[], item) => {
